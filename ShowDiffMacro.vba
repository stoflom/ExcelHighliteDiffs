...
'The subroutine caters for a combined total of MaxDiffs tags per cell.
'
...
Sub TurnTextRedBlue()
    ...
    Dim var As String
    Dim updates(MaxDiffs, 2) As Long 'Array of (start,length, del=0/ins=1) updates
    ...
    SaveMod = 10000 'Save every 1000 rows
    ...
    Do While progress < StopStop
        ...
        If Len(var) > 11 Then 'Must have at least <ins></ins> (11 chars)
            ...
            Do
                If numUpdates > MaxDiffs Then
                    MsgBox "ERROR: Maximum number of tags exceeded!"
                    Exit Sub
                End If
